...
{
	ATF_CHECK_EQ((IBS_COMP_TO_RET_CTR & IBS_TAG_TO_RET_CTR), 0ULL);
}
...
ATF_TC_BODY(ibs_unit_comp_extraction, tc)
{
	...
	ATF_CHECK_EQ(comp, 0xBEEFULL);
	ATF_CHECK_EQ(tag, 0xDEADULL);
}
...
ATF_TC_BODY(ibs_unit_brn_bits_consecutive, tc)
{
	...
	ATF_CHECK_EQ(IBS_OP_BRN_TAKEN, IBS_OP_RETURN << 1);
	ATF_CHECK_EQ(IBS_OP_BRN_MISP,  IBS_OP_RETURN << 2);
	ATF_CHECK_EQ(IBS_OP_BRN_RET,   IBS_OP_RETURN << 3);
}
